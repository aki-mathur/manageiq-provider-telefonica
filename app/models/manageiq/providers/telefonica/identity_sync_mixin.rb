module ManageIQ::Providers::Telefonica::IdentitySyncMixin
  ...
  def new_users
    users = []
    telefonica_users = list_users
    telefonica_users.each do |u|
      username = u["name"]
      user_uuid = u["id"]
  ...
  def keystone
    telefonica_handle.identity_service
  end
  ...
  def create_or_find_user(telefonica_uuid, username, email, password_digest)
    user = User.find_by(:userid => username)
    if user
      ...
      user.settings[:telefonica_user_id] = telefonica_uuid
      user.save!
    end
    user
  end
